...
class TikTokService {
    ...
    async init() {
        if (!this.browser) {
            try {
                ...
                logger_1.logger.error(`Failed to launch browser: ${error.message}`);
    ...
    async getDirectUrl(tiktokUrl) {
        ...
        try {
            logger_1.logger.info(`Fetching direct URL for: ${tiktokUrl}`);
            ...
            logger_1.logger.success(`Direct URL obtained: ${directUrl}`);
            return {
                directUrl,
                title: title.trim(),
                author: author.trim()
            ...
            logger_1.logger.error(`Failed to get direct URL: ${error}`);
            throw error;
        }
        finally {
            await page.close();
